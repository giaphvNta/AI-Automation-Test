...
>
> Sau khi đọc RULES.md, xuất ngay: `✅ RULES đã đọc | Nắm: #1 #2 ... #16`
...
```bash
bash /home/user/ai-automation-test/automation/scripts/update-status.sh <step> <key> "<project>" "<msg>" [heal] [mode] [status]
```
...
```
Read /home/user/ai-automation-test/skills/ai-test/rules/RULES.md
```
...
> 📊 `update-status.sh 1 s1 "<name>" "Parsing arguments..." 0 normal`
...
- `--live` — **set `TEST_LIVE=1`** (headed mode + VNC tại http://localhost:6080/vnc.html)
...
> 📊 `update-status.sh 1 s1b "<name>" "Detecting spec changes..." 0 normal`
...
> 📊 `update-status.sh 2 s2 "<name>" "Switching context..." 0 normal`
...
> 📊 `update-status.sh 3 s3 "<name>" "Loading input + extracting scenarios..." 0 normal`
...
> 📊 `update-status.sh 3 s3b "<name>" "Checking and seeding test data..." 0 normal`
...
> 📊 `update-status.sh 4 s4 "<name>" "Generating test plan..." 0 normal`

Nếu có Agent `playwright-test-planner` → dùng Agent tool.
Fallback: tự sinh `projects/<name>/specs/<slug>.md` với format: scenario title, pre-conditions, steps, expected outcomes, selector hints.
...
> 📊 `update-status.sh 5 s5 "<name>" "Generating test code..." 0 normal`

Nếu có Agent `playwright-test-generator` → dùng Agent tool.
Fallback: tự viết `projects/<name>/tests/<slug>.spec.ts`.
...
> 📊 `update-status.sh 6 s6 "<name>" "Running tests in Docker..." 0 normal`
...
# --only: ... --grep "<test>"
# Heal rerun: TEST_RUN_ID=<id> SPEC_FILE=... ./scripts/run-test.sh <name>
```
...
while heal_count < max_heal AND có fail:
  ...
  1. Đọc results.json → lấy field "errors"/"message" của test fail
  2. Phân loại:
  ...
  3. Patch → rerun cùng TEST_RUN_ID
  heal_count += 1
```
...
> 📊 `update-status.sh 6 s6b "<name>" "Converting videos..." 0 normal`
...
docker compose run --rm --entrypoint bash playwright -c "
  ...
  if [ -s \"\$CONCAT_LIST\" ]; then
    ffmpeg -f concat -safe 0 -i \"\$CONCAT_LIST\" \
      ...
      \"\$RESULTS/full-session.mp4\" -y 2>/dev/null && echo 'full-session.mp4 OK'
  fi
  rm -f \"\$CONCAT_LIST\"
"
```
...
> 📊 `update-status.sh 7 s7 "<name>" "Generating report..." 0 normal`
> Xong hết: `update-status.sh 7 s7 "<name>" "Done" 0 normal done`
